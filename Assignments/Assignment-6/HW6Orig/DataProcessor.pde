...
{  
  ...
  {    
    ...
    skip_reduction = skip_every_n/10;
  ...
  int pow (int a, int b)
{
  ...
  {    
    println("Run called.");
    while(this.getDataPoints().length != this.getInputSize() && (Thread.currentThread() == thread))
    {
        churn();
        try {
          Thread.sleep(1);
  ...
  {    
    ...
    synchronized(processedData)
    {
        ...
        skip_every_n = skip_every_n - 1000; 
        //skip_every_n /= 2; 
    }
  }
  ...
  public float[] getDataPoints()
  {
    ...
    if(processedData == null)
        return new float[1];
    {
        println("Sending "+ processedData.length + " points.");      
